...
Q_COU as
 (select LOC.ID as LOC_ID,
    LOCA1.CHAR_VALUE as ISO3166_NUMERIC_CODE, LOCA2.CHAR_VALUE as ISO3166_ALPHA3_CODE, LOC.ITM_ID,
...
--
select cast(POC.ASR_YEAR as NUMBER(4)) as ASR_YEAR,
  cast(nvl(COU1.ISO3166_NUMERIC_CODE, COU1.ISO3166_ALPHA3_CODE) as varchar2(3))
    as COU_CODE_RESIDENCE,
  cast(COU1.ISO3166_ALPHA3_CODE as varchar2(3)) as ISO3166_ALPHA3_CODE_RESIDENCE,
  ...
  cast(nvl(COU2.ISO3166_NUMERIC_CODE, COU2.ISO3166_ALPHA3_CODE) as varchar2(3)) as COU_CODE_ORIGIN,
  cast(COU2.ISO3166_ALPHA3_CODE as varchar2(3)) as ISO3166_ALPHA3_CODE_ORIGIN,
...
from
 (select POC.ASR_YEAR,
    case  -- Treat "Stateless" as "Various" for country of residence
      when nvl(POC.LOC_ID_RESIDENCE, LOC2.ID) = LOC2.ID then LOC1.ID
      else POC.LOC_ID_RESIDENCE
    end as LOC_ID_RESIDENCE,
    nvl(POC.LOC_ID_ORIGIN, LOC1.ID) as LOC_ID_ORIGIN,
    POC.POP_TYPE_CODE, POC.REDACTION_LIMIT, POC.VALUE
  from Q_POC POC
  cross join
   (select LOC.ID
    from T_LOCATIONS LOC
    inner join T_LOCATION_ATTRIBUTES LOCA
      ...
      and LOCA.CHAR_VALUE = 'XXX'
    where LOCT_CODE = 'OTHORIGIN') LOC1
  cross join
   (select LOC.ID
    from T_LOCATIONS LOC
    inner join T_LOCATION_ATTRIBUTES LOCA
      on LOCA.LOC_ID = LOC.ID
      and LOCA.LOCAT_CODE = 'ISO3166A3'
      and LOCA.CHAR_VALUE = 'XXA'
    where LOCT_CODE = 'OTHORIGIN') LOC2) POC
...
select ASR_YEAR,
  COU_CODE_RESIDENCE, ISO3166_ALPHA3_CODE_RESIDENCE, ITM_ID_RESIDENCE,
  ...
  COU_CODE_ORIGIN, ISO3166_ALPHA3_CODE_ORIGIN, ITM_ID_ORIGIN,
...
select POC.ASR_YEAR,
  ...
  nvl(TXT1.TEXT, NAM1.NAME) as COU_NAME_RESIDENCE,
  POC.UNSD_GSR_ITM_ID_RESIDENCE, POC.UNSD_GSR_ORDER_SEQ_RESIDENCE,
  POC.UNSD_MGR_ITM_ID_RESIDENCE, POC.UNSD_MGR_ORDER_SEQ_RESIDENCE,
  POC.UNHCR_ROP_ITM_ID_RESIDENCE, POC.UNHCR_ROP_ORDER_SEQ_RESIDENCE,
  POC.UNHCR_BUR_ITM_ID_RESIDENCE, POC.UNHCR_BUR_ORDER_SEQ_RESIDENCE,
  POC.COU_CODE_ORIGIN, POC.ISO3166_ALPHA3_CODE_ORIGIN,
  coalesce(TXT2.TEXT, TXT3.TEXT, NAM2.NAME) as COU_NAME_ORIGIN,
  POC.UNSD_GSR_ITM_ID_ORIGIN, POC.UNSD_GSR_ORDER_SEQ_ORIGIN,
  POC.UNSD_MGR_ITM_ID_ORIGIN, POC.UNSD_MGR_ORDER_SEQ_ORIGIN,
  POC.UNHCR_ROP_ITM_ID_ORIGIN, POC.UNHCR_ROP_ORDER_SEQ_ORIGIN,
  POC.UNHCR_BUR_ITM_ID_ORIGIN, POC.UNHCR_BUR_ORDER_SEQ_ORIGIN,
...
left outer join T_TEXT_ITEMS TXT1
  on TXT1.ITM_ID = POC.ITM_ID_RESIDENCE
  and TXT1.TXTT_CODE = 'PSRNAME'
  and TXT1.SEQ_NBR = 1
  and TXT1.LANG_CODE = NAM1.LANG_CODE
left outer join NAMES NAM2
  on NAM2.ITM_ID = POC.ITM_ID_ORIGIN
left outer join T_TEXT_ITEMS TXT2
  on TXT2.ITM_ID = POC.ITM_ID_ORIGIN
  and TXT2.TXTT_CODE = 'ORIGINNAME'
  and TXT2.SEQ_NBR = 1
  and TXT2.LANG_CODE = NAM2.LANG_CODE
left outer join T_TEXT_ITEMS TXT3
  on TXT3.ITM_ID = POC.ITM_ID_ORIGIN
  and TXT3.TXTT_CODE = 'PSRNAME'
  and TXT3.SEQ_NBR = 1
  and TXT3.LANG_CODE = NAM2.LANG_CODE
left outer join
