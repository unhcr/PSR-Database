...
CREATE TABLE T_DATA_ITEM_PERMISSIONS 
    ( 
     ...
     UPDATE_USERID VARCHAR2 (31) DEFAULT nvl(sys_context(P_CONTEXT.GET_CONTEXT_NAME(), 'USERID'), '*' || user)  NOT NULL 
    ) 
;
...
FROM
  ...
  AND ULP.USERID   = sys_context(P_CONTEXT.GET_CONTEXT_NAME(), 'USERID')
  )
WHERE RANK = 1 ;
...
FROM
  ...
  WHERE UIRC.USERID   = sys_context(P_CONTEXT.GET_CONTEXT_NAME(), 'USERID')
  UNION ALL
  ...
  WHERE EXISTS
    ...
    WHERE T_USERS_IN_ROLES.USERID = sys_context(P_CONTEXT.GET_CONTEXT_NAME(), 'USERID')
    )
  ) ;
...
FROM
  ...
  WHERE UIRC.USERID = sys_context(P_CONTEXT.GET_CONTEXT_NAME(), 'USERID')
  AND UIRC.LOC_ID   = to_number(sys_context(P_CONTEXT.GET_CONTEXT_NAME(), 'COUNTRY'))
  UNION ALL
  SELECT UIR.ROL_ID,
    ROL.DESCRIPTION AS ROL_DESCRIPTION,
    ROL.LANG_CODE   AS ROL_LANG_CODE,
  ...
  WHERE UIR.USERID = sys_context(P_CONTEXT.GET_CONTEXT_NAME(), 'USERID')
  ) ;
